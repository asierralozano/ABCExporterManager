...
logger = logging.getLogger("ABCExporter")

__all__ = [
    "abc_export"
]

def abc_export(dag_node,
               folder_path=None,
               frame_range=None,
    ...
    # abc_args = ["-selection"]
    abc_args = ["-root {}".format(dag_node)]

    if not folder_path:
        folder_path = utils.get_cache_folder()

    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    
    abc_file_path = utils.generate_abc_path(folder_path, dag_node)
    abc_args.append("-file {}".format(abc_file_path))

    if not frame_range or not isinstance(frame_range, tuple):
        frame_range = utils.get_frame_range()
    abc_args.append("-frameRange {} {}".format(frame_range[0], frame_range[1]))

    if renderable_only:
        abc_args.append("-renderableOnly")

    if world_space:
        abc_args.append("-worldSpace")

    MC.select(clear=True)
    MC.select(dag_node)

    abc_args_string = " ".join(abc_args)

    try:
        logger.error("Exporting {}...".format(dag_node))
        MC.AbcExport(jobArg=abc_args_string)
        logger.error("Alembic File created in {}".format(abc_file_path))
        return True
    except:
        traceback.print_exc()
        logger.error("Rolling back ABC Exporting! Deleting file!")
